    ...
    >
      <template v-if="!(relationGraph.options.hideNodeContentByZoom === true && relationGraph.options.canvasZoom<40)">
        <slot name="node" :node="nodeProps">
          <div v-if="!nodeProps.innerHTML" :style="{'color':(nodeProps.fontColor || relationGraph.options.defaultNodeFontColor)}" class="c-node-text">
...
export default {
  ...
  methods: {
    ...
    onDragStart(e) {
      if (this.relationGraph.options.disableDragNode || this.nodeProps.disableDrag) {
        return;
      }
      this.dragging = true;
      this.hovering = false;
    ...
    onNodeDraged(x_buff, y_buff, e) {
      ...
      if (this.relationGraph.options.isMoveByParentNode) {
